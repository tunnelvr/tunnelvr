...
func get_current_player_height():
	return HeadCam.global_transform.origin.y - HeadCam.get_parent().global_transform.origin.y;

func initjogdetectionsystem(lHeadCam):
	HeadCam = lHeadCam
	_height_ringbuffer.resize(_height_ringbuffer_size);
	_current_height_estimate = get_current_player_height();
...
func _store_height_in_buffer(y):
	...
	if _height_ringbuffer_pos == 0:
		print(_height_ringbuffer)
...
func _process(dt):
	...
	if k == 0:
		_time_since_last_step += dt;
		return
	var headset_height = get_current_player_height();
	var corrected_height = _get_viewdir_corrected_height(headset_height, -HeadCam.transform.basis.z.y);
